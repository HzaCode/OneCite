...
def test_noninteractive_low_confidence_candidate_stays_unresolved():
    identifier = IdentifierModule()
    raw_entry = {
        "id": 0,
        "raw_text": "ambiguous citation",
        "query_string": "ambiguous citation",
    }
    ...
    with (
        patch.object(identifier, "_search_crossref", return_value=[low_confidence_candidate]),
        patch.object(identifier, "_search_semantic_scholar", return_value=[]),
        patch.object(identifier, "_score_candidates", return_value=[low_confidence_candidate]),
    ):
        result = identifier._fuzzy_search(raw_entry, interactive_callback=lambda _candidates: -1)
...
def test_plain_text_query_is_unresolved_in_process_identifier_path():
    ...
    with patch.object(identifier, "_fuzzy_search") as fuzzy_search:
