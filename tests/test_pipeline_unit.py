...
class TestIdentifierThesis:
    ...
    def test_openaire_search(self):
        ...
        with patch("onecite.pipeline.requests.get", side_effect=fake_get):
            r = ident._search_openaire_for_thesis("OpenAIRE Thesis", year=2021)

        assert r["source"] == "openaire"
        assert r["title"] == "OpenAIRE Thesis"
        assert r["school"] == "OpenAIRE University"
    ...
    def test_manual_fallback(self):
        """When both OpenAIRE and BASE return nothing we parse the text ourselves."""
        ident = IdentifierModule()
