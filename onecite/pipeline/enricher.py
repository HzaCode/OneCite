...
class EnricherModule:
    ...
    def enrich(
        ...
        if raw_entries:
            ...
            self.logger.warning(
                "raw_entries is None or empty - original fields will not be preserved!"
            )
        ...
        self.logger.info(
            f"Enrichment completed: {successful_count}/{len(completed_entries)} entries successfully completed"
        )
    ...
    def _enrich_single_entry(
        ...
        try:
            ...
            # Check if it's repository/software or dataset metadata that
            # already came from a source-specific API.
            if metadata.get("source") in ["github", "zenodo", "figshare", "datacite"]:
            ...
            is_thesis_type = (
                metadata.get("is_thesis") is True
                or metadata.get("type") in ["phdthesis", "mastersthesis", "thesis"]
                or base_record.get("is_thesis") is True
            )

            is_software_type = (
                metadata.get("is_software") is True
                or metadata.get("type") == "software"
                or base_record.get("is_software") is True
            )

            is_dataset_type = (
                metadata.get("is_dataset") is True
                or metadata.get("type") == "dataset"
                or base_record.get("is_dataset") is True
            )

            is_book_type = (
                metadata.get("is_book") is True
                or str(metadata.get("is_book")).lower() == "true"
                or metadata.get("type") in ["book", "monograph", "edited-book", "reference-book"]
                or base_record.get("is_book") is True
                or str(base_record.get("is_book")).lower() == "true"
            )
    ...
    def _convert_search_metadata(self, metadata: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            is_thesis = metadata.get("is_thesis") or metadata.get("type") in [
                "phdthesis",
                "mastersthesis",
                "thesis",
            ]
            ...
            is_book = metadata.get("is_book") or metadata.get("type") in [
                "book",
                "monograph",
                "edited-book",
                "reference-book",
    ...
    def _complete_fields(
        ...
        allow_pubmed_fallback: Optional[bool] = None,
    ) -> Dict:
        ...
        if allow_abstract_fallback and not record.get("abstract") and record.get("doi"):
    ...
    def _get_semantic_scholar_abstract(self, doi: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 429:
                self.logger.debug(
                    "Semantic Scholar rate-limited (429); skipping abstract fallback."
                )
                return None
    ...
    def _get_pubmed_abstract(self, base_record: Dict) -> Optional[str]:
        ...
        Zhang 2020 example DOI 10.1007/s10462-019-09792-7 pulled the
            ...
            if pmid:
                ...
                if abstract_elems:
                    ...
                    if parts:
                        abstract = " ".join(parts)
                        self.logger.info(
                            f"Successfully retrieved abstract from PubMed (PMID: {pmid})"
                        )
                        return abstract
